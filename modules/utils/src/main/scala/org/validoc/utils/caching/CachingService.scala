...
class CachingService[M[_] : Async, Req, Id, Res](val name: String, delegate: Service[M, Req, Res],
                                                 cachingStrategy: CachingStrategy[M, Req, Id, Res], sizeStrategy: MapSizeStrategy) extends Service[M, Req, Res] with CachingOps with Logging {

  import cachingStrategy._

  private implicit val async = implicitly[Async[M]]
  private val nextId = new AtomicLong()
  private val metrics = CachingMetrics()
  private val map = SafeMap[Id, CachedValue[M, Res]](CachedValue[M, Res](nanoTimeService(), CachedId(nextId.getAndIncrement()), None, None), sizeStrategy, metrics)
  ...
  private def askDelegate(req: Req, c: CachedValue[M, Res]) = {
    ...
    val finalResult = async.registerSideEffectWhenComplete(delegateResult, recordResult(req, c))
  ...
  private def sendToDelegateIfNeededUpdatingCache(req: Req)(c: CachedValue[M, Res]): CachedValue[M, Res] = {
    ...
    (c, staleState(c)) match {
      ...
      case (c@CachedValue(_, _, None, Some(future)), Stale) =>
        ...
        metrics.staleRequest.incrementAndGet()
        askDelegate(req, c)
  ...
  private def cachedRequest(req: Req): M[Res] =
    map(id(req)) {
      sendToDelegateIfNeededUpdatingCache(req)
    }.valueToUse
