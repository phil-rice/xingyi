...
case class NotFoundResult[Result](contentType: ContentType, legalContentTypes: Set[ContentType]) extends ParserResult[Result] {
...
trait ParserFinder[T] extends ((ContentType, String) => ParserResult[T]) {
  ...
  override def apply(v1: ContentType, v2: String) = find(v1).map(_ (v2))
}
