...
abstract class AbtractFastOrmBulkSpecWithDeepNestingSpec[M[_] : ClosableM, J: JsonParser, DS <: DataSource](implicit jdbcOps: JdbcOps[DataSource]) extends SharedOrmFixture with SetupDatabaseForOrmFixture[DS] with DatabaseSourceFixture[DS] {
  ...
  def setupSchema(s1: Boolean, s2: Boolean, s3: Boolean, s4: Boolean)(fn: (OrmMaker[String], OrmFactory[SchemaForTest], OrmKeys[SchemaForTest], SchemaForTest[_], SchemaForTest[_], SchemaForTest[_], SchemaForTest[_]) => Unit): Unit = {
    ...
    implicit def toTableName[E <: OrmEntity](e: EntityAndFieldsAndPath[E]): TableName = e.entity.tableName
    implicit def toTableNameString[E <: OrmEntity](e: EntityAndFieldsAndPath[E]): String = e.tableName
    ...
    lazy val ormFactory: OrmFactory[SchemaForTest] = tablesAndFieldsAndPaths.ormFactory(keysForT1)

    fn(maker, ormFactory, keysForT1, schema1, schema2, schema3, schema4)
  }

  behavior of "deeply nested one to many"

  it should "be loadable" in {
    setupSchema(true, true, true, true) { (maker, ormFactory, keysForT1, schema1, schema2, schema3, schema4) =>
      implicit val ormMaker = maker

      val entity4: EntityAndFieldsAndPath[OneToManyEntity] = ormFactory.oneToManyEntity(table4, "a4", Keys("t4id:int"), Keys("parentId:int"), List())
      ...
      setup(ds, mainEntity.entity) {
        ...
        mainEntity.entity.stream[String](OrmBatchConfig(ds, 3)).toList shouldBe List(
  ...
  def setupNestedManyToOne(fn: (EntityAndFieldsAndPath[MainEntity], OrmKeys[SchemaForTest]) => OrmMaker[String] => Unit): Unit = {
    setupSchema(false, false, false, false) { (maker, ormFactory, keysForT1, schema1, schema2, schema3, schema4) =>
      ...
      setup(ds, mainEntity.entity) {
        ...
        fn(mainEntity, keysForT1)(maker)
      }
    }
  }

  it should "have a bulk pointer" in {
    setupNestedManyToOne { (mainEntity, keys) =>
      implicit ormMaker =>
        val data: Map[OrmEntity, List[List[Any]]] = FastReader.getOneBlockOfDataFromDs(ds, mainEntity.entity, 2)(0)
        lazy val mainBulkData = MainBulkData(mainEntity.entity, data)
  ...
  it should "be loadable" in {
    setupSchema(false, true, false, true) { (maker, ormFactory, keysForT1, schema1, schema2, schema3, schema4) =>
      implicit val ormMaker = maker
