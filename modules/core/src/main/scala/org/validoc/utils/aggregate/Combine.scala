...
class Combine2[M[_], Req, Res1, Res2](k1: Req => M[Res1], k2: Req => M[Res2]) {
  def mergeInto[Res](merger: (Req, Res1, Res2) => Res)(implicit asyncMonad: AsyncMonad[M]): (Req => M[Res]) = { req =>
    val m1 = k1(req)
    val m2 = k2(req)
...
class Combine3[M[_], Req, Res1, Res2, Res3](k1: Req => M[Res1], k2: Req => M[Res2], k3: Req => M[Res3]) {
  def mergeInto[Res](merger: (Req, Res1, Res2, Res3) => Res)(implicit asyncMonad: AsyncMonad[M]): (Req => M[Res]) = { req =>
    val m1 = k1(req)
    val m2 = k2(req)
    val m3 = k3(req)
...
class Combine4[M[_], Req, Res1, Res2, Res3, Res4](k1: Req => M[Res1], k2: Req => M[Res2], k3: Req => M[Res3], k4: Req => M[Res4]) {
  def mergeInto[Res](merger: (Req, Res1, Res2, Res3, Res4) => Res)(implicit asyncMonad: AsyncMonad[M]): (Req => M[Res]) = { req =>
    val m1 = k1(req)
    val m2 = k2(req)
    val m3 = k3(req)
    val m4 = k4(req)
