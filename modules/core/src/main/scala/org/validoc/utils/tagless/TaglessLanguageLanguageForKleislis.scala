...
class TaglessLanguageLanguageForKleislis[M[_] : Async, Fail] extends CommonForKleislis[M] {
  ...
  case class NonFunctionalLanguageService(implicit monadCanFail: MonadCanFailWithException[M, Fail],
    ...
    override def profile[Req: ClassTag, Res: ClassTag](profileData: TryProfileData)(raw: Kleisli[Req, Res]): Req => M[Res] =
      raw.onEnterAndExitM(_ => timeService(), profileData.eventFromStartTime)


    override def enrichPrim[ReqP, ResP, ReqC, ResC, ResE](parentService: Kleisli[ReqP, ResP], childService: Kleisli[ReqC, ResC])(implicit findChildIds: HasChildren[ResP, ReqC], enricher: Enricher[ReqP, ResP, ReqC, ResC, ResE]): ReqP => M[ResE] =
      parentService |=++> { reqP => resP => findChildIds ~+> childService |=> (seq => enricher(reqP, resP, seq)) }


    override def merge2Prim[ReqM, ResM, Req1, Res1, Req2, Res2](firstService: Kleisli[Req1, Res1], secondService: Kleisli[Req2, Res2], merger: (ReqM, Res1, Res2) => ResM)(implicit reqMtoReq1: ReqM => Req1, reqMtoReq2: ReqM => Req2): ReqM => M[ResM] =
      join2WithReq(
        reqMtoReq1 ~> firstService,
        reqMtoReq2 ~> secondService
      ) |=> merger.tupled

    override def merge3Prim[ReqM, ResM, Req1, Res1, Req2, Res2, Req3, Res3](firstService: Kleisli[Req1, Res1], secondService: Kleisli[Req2, Res2], thirdService: Kleisli[Req3, Res3], merger: (ReqM, Res1, Res2, Res3) => ResM)(implicit reqMtoReq1: ReqM => Req1, reqMtoReq2: ReqM => Req2, reqMtoReq3: ReqM => Req3): ReqM => M[ResM] =
      join3WithReq(
        reqMtoReq1 ~> firstService,
        reqMtoReq2 ~> secondService,
