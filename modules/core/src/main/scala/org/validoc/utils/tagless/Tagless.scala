...
trait NonfunctionalLanguage[Wrapper[_, _]] {
  ...
  def caching[Req: ClassTag : CachableKey, Res: CachableResult : ClassTag](name: String, cachingStrategy: StaleCacheStrategy, sizeStrategy: MapSizeStrategy)(raw: Wrapper[Req, Res]): Wrapper[Req, Res]
...
trait ComposeLanguage[Wrapper[_, _]] {
  def enrich[ReqP, ResP, ReqC, ResC, ResE](parent: Wrapper[ReqP, ResP], child: Wrapper[ReqC, ResC])(implicit findChildIds: HasChildren[ResP, ReqC], enricher: Enricher[ResE, ResP, ResC]): Wrapper[ReqP, ResE]
...
class TaglessLanguageLanguageForKleislis[M[_] : Async] {
  ...
  class NonFunctionalLanguageService(implicit timeService: NanoTimeService, putMetrics: PutMetrics, loggingAdapter: LoggingAdapter) extends NonfunctionalLanguage[K] {
    ...
    override def caching[Req: ClassTag : CachableKey, Res: CachableResult : ClassTag](name: String, cachingStrategy: StaleCacheStrategy, sizeStrategy: MapSizeStrategy)(raw: K[Req, Res]): K[Req, Res] =
...
class TaglessLanguageForToString[HttpReq, HttpRes] {
  ...
  implicit object ForToString extends TaglessLanguage[StringHolder, HttpReq, HttpRes] {
    ...
    override def caching[Req: ClassTag : CachableKey, Res: CachableResult : ClassTag](name: String, cachingStrategy: StaleCacheStrategy, sizeStrategy: MapSizeStrategy)(raw: StringHolder[Req, Res]): StringHolder[Req, Res] =
      raw.addLineAndIndent(s"caching($name)")
    override def enrich[ReqP, ResP, ReqC, ResC, ResE](parent: StringHolder[ReqP, ResP], child: StringHolder[ReqC, ResC])(implicit findChildIds: HasChildren[ResP, ReqC], enricher: Enricher[ResE, ResP, ResC]): StringHolder[ReqP, ResE] =
