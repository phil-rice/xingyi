...
) extends LiftFunctionKleisli[M] with MicroserviceComposers[M] with EndpointKleisli[M]
  with RecordCallsKleisli[M, Fail] with DisplayRecordedKleisli[M] with ChainKleisli[M, Fail]