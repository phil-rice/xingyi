...
trait HasParentF[F[_]] {def apply[T](t: F[T]): Option[F[_]]}
...
trait HigherOrderAccessorsLanguage {
  ...
  implicit class HasParentOps[F[_], T](f: F[T])(implicit hasParent: HasParentF[F]) {def parents: Option[F[_]] = hasParent(f)}
  implicit class HasParentsOps[F[_], T](f: F[T])(implicit hasParents: HasParentsF[F]) {def parents: List[F[_]] = hasParents(f)}
